...
本地使用时让 JobPilot 运行在 `3000` 端口；Vercel 版本则填写部署后的 HTTPS 根地址（当前公开版为 `https://try-jobpilot.vercel.app`），然后打开“设置 → Chrome 一键保存”：

1. 下载并解压扩展。
2. 打开 `chrome://extensions`。
3. 开启“开发者模式”，点击“加载已解压的扩展程序”。
4. Vercel 下载版会自动使用部署地址，不需要填写 URL；只需粘贴同一账户在 JobPilot 页面生成的配对令牌。使用本地/开源构建时，才在 **JobPilot URL** 中填写本地或部署后的根地址，不要填写 `/extension`、`/matches` 或 API 路径。Chrome 如果请求访问托管站点的权限，请允许。
