...
test("routes resume feedback questions to the AI instead of the fixed Resume Studio guide", () => {
  ...
  assert.equal(isResumeAdviceRequest("我觉得我的简历还有很多需要调整的地方，你看看有什么建议吗"), true);
  assert.equal(isResumeAdviceRequest("不是说事实性内容，我是在说展示方式"), true);
  assert.equal(isResumeAdviceRequest("我不是在说内容，你能看到我的简历吗"), true);
  assert.equal(isExplicitResumeMutationRequest("帮我修改简历里的 JobPilot 项目"), true);
  assert.equal(isExplicitResumeMutationRequest("我觉得我的简历还有很多需要调整的地方，你看看有什么建议吗"), false);
  assert.equal(isExplicitResumeMutationRequest("请给我一些简历优化建议"), false);
  assert.equal(isExplicitResumeMutationRequest("请优化我的简历"), true);
  assert.equal(isExplicitResumeMutationRequest("简历可以优化吗？"), false);
  assert.equal(isExplicitResumeMutationRequest("请问简历怎么优化？"), false);
  assert.equal(isExplicitResumeMutationRequest("帮我查看一下简历并说说哪里需要修改"), false);
  assert.equal(isExplicitResumeMutationRequest("请帮我把简介改得更简洁"), true);
  assert.equal(isExplicitResumeMutationRequest("Please give me resume feedback"), false);
  assert.equal(isExplicitResumeMutationRequest("Please revise my resume"), true);
...
test("inherits the active resume task for natural contextual follow-ups", () => {
  ...
  const switchedFromSync: AssistantChatMessage[] = [
    { role: "user", content: "比较我的中英文简历" },
    { role: "assistant", content: "我可以同步两份简历的差异。", intent: "resume_sync", awaitingReply: true },
    { role: "user", content: "我不是在说内容，你能看到我的简历吗" },
  ];
  assert.equal(resolveAssistantActiveIntent(switchedFromSync, 2), "resume_advice");
});
