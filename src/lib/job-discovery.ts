...
export async function saveDiscoveredJob(userId: string, item: NormalizedJob, source: { type: "manual" | "extension" | "search"; name: string; evidence: string; rawSnapshotText?: string }, database: typeof db = db) {
  const now = new Date();
  const [user, preference, targets] = await Promise.all([
    database.select().from(users).where(eq(users.id, userId)).get(),
    database.select().from(careerPreferences).where(eq(careerPreferences.userId, userId)).get(),
    database.select().from(jobSearchTargets).where(eq(jobSearchTargets.userId, userId)).all(),
  ]);
  ...
  // An ignore is an automatic-discovery exclusion. A user-initiated manual
  // import or extension save is an explicit recovery path after a misclick.
  if (source.type === "search" && await isJobIgnored(userId, canonicalKey, database)) return { jobId: null, created: false, hardFilterPassed: false, score: 0, ignored: true as const };
  const existing = await database.select().from(jobs).where(and(eq(jobs.ownerUserId, userId), eq(jobs.canonicalKey, canonicalKey))).get();
