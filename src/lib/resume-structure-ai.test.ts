...
import { applyResumeSectionTemplate, buildGroundedPlatformResume, indexedResultToResumeResult, isRecoverableResumeStructureError, preserveUnmappedResumeSource, type ResumeStructureAiResult } from "@/lib/resume-structure-ai";
...
test("classifies truncated or malformed AI structure output as safely recoverable", () => {
  assert.equal(isRecoverableResumeStructureError(new Error("DeepSeek structured output remained truncated after JobPilot's compact retry.")), true);
  assert.equal(isRecoverableResumeStructureError(new Error("OpenAI returned incomplete structured data after one repair attempt.")), true);
  assert.equal(isRecoverableResumeStructureError(new Error("DeepSeek request timed out.")), true);
  assert.equal(isRecoverableResumeStructureError(new Error("DeepSeek returned HTTP 429: rate limit")), false);
  assert.equal(isRecoverableResumeStructureError(new Error("DeepSeek API key is not configured.")), false);
});
...
test("recovers omitted project and work identity fields from referenced source lines", () => {
  const sourceLines = [
    { id: 1, text: "EXPERIENCE & PROJECTS" },
    { id: 2, text: "CorpOS    Apr. 2026 - Present" },
    { id: 3, text: "Developed a locally-first multi-agent runtime with audited tool calls." },
    { id: 4, text: "Tech stack: Python, Pydantic, SQLite" },
    { id: 5, text: "JobPilot    Jul. 2026 - Present" },
    { id: 6, text: "Built a local-first AI job search platform." },
    { id: 7, text: "Research Assistant (Artificial Intelligence)    Nov. 2025 - Present" },
    { id: 8, text: "University of New South Wales" },
    { id: 9, text: "Sydney/NSW" },
    { id: 10, text: "• Built an evaluation pipeline." },
  ];
  const mapped = indexedResultToResumeResult(sourceLines, {
    basics: {},
    sections: [{
      type: "experience_projects",
      sourceLabelLineId: 1,
      entries: [
        { sourceLineIds: [3], bodyLineIds: [2, 4] },
        { sourceLineIds: [5], bodyLineIds: [6] },
        { sourceLineIds: [7], bodyLineIds: [10] },
      ],
    }],
  });
  const entries = mapped.sections[0]?.entries ?? [];

  assert.equal(entries[0]?.projectName, "CorpOS");
  assert.equal(entries[0]?.category, "project");
  assert.doesNotMatch(entries[0]?.description ?? "", /^CorpOS/);
  assert.equal(entries[1]?.projectName, "JobPilot");
  assert.equal(entries[1]?.category, "project");
  assert.equal(entries[2]?.position, "Research Assistant (Artificial Intelligence)");
  assert.equal(entries[2]?.organization, "University of New South Wales");
  assert.equal(entries[2]?.location, "Sydney/NSW");
  assert.equal(entries[2]?.category, "experience");
});

test("recovers education degree, school, and location when indexed fields are omitted", () => {
  const mapped = indexedResultToResumeResult([
    { id: 1, text: "EDUCATION" },
    { id: 2, text: "Master of Information Technology (Artificial Intelligence)    Feb. 2024 - Dec. 2025" },
    { id: 3, text: "University of New South Wales    Sydney/NSW" },
    { id: 4, text: "WAM: 88.7/100" },
  ], {
    basics: {},
    sections: [{ type: "education", sourceLabelLineId: 1, entries: [{ sourceLineIds: [2], bodyLineIds: [4] }] }],
  });
  const entry = mapped.sections[0]?.entries[0];

  assert.equal(entry?.degree, "Master of Information Technology (Artificial Intelligence)");
  assert.equal(entry?.fieldOfStudy, "Artificial Intelligence");
  assert.equal(entry?.school, "University of New South Wales");
  assert.equal(entry?.location, "Sydney/NSW");
});
