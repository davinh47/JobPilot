...
import { localeFromStored, type Locale } from "@/lib/i18n";
...
export async function runWebJobSearch(userId: string, localeOverride?: Locale) {
  ...
  if (!latestPlan && !isCloudDeployment) {
    latestPlan = await generateSearchStrategy(userId, locale).catch(() => undefined);
  ...
  try {
    ...
    if (settings.aiEnabled && settings.webAiMatchLimit > 0) {
      ...
      const matchOutcomes = await mapWithConcurrency(selected, limits.aiMatchConcurrency, async (candidate) => {
        ...
        try {
          await analyzeJobMatchById(userId, candidate.jobId, { timeoutMs: boundedTimeout(limits.aiTimeoutMs, 15_000), locale });
