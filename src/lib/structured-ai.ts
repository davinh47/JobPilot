...
export async function requestStructuredAiJsonWithKey<T>({ apiBaseUrl, model, system, user, schema, outputMode = "compact", maxOutputTokens, timeoutMs = 120_000, apiKey, fetcher = fetch, userId, agentRunId, taskType = "structured", promptVersion, ...request }: StructuredRequest<T> & { apiKey: string; fetcher?: typeof fetch }) {
  ...
  async function complete(messages: Message[], outputTokens = initialOutputTokens) {
    ...
    try {
      ...
      body: JSON.stringify(isOpenAi
        ...
        : { model, messages, response_format: { type: "json_object" }, max_tokens: outputTokens, stream: false }),
