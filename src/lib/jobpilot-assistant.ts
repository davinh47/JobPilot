...
export function resolveAssistantActiveIntent(messages: AssistantChatMessage[], latestUserIndex: number) {
  ...
  if (isResumeAdviceRequest(latest)) return "resume_advice" as const;
  return undefined;
}
...
export function isResumeAdviceRequest(message: string) {
  return /简历|履历|resume|cv/i.test(message)
    && /建议|评价|评估|检查|问题|不足|优点|缺点|改进|优化|怎么样|如何写|怎么写|应该|适合|保留|删除|修改|更新|改写|重写|润色|添加|增加|补充|翻译|比较|同步|生成|review|advice|feedback|evaluate|improve|weakness|strength|edit|update|rewrite|revise|polish|add|translate|compare|sync|generate|tailor|ats/i.test(message);
}
