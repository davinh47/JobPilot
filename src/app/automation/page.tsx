...
export default async function AutomationPage({ searchParams }: { searchParams: Promise<{ mode?: string }> }) {
  ...
  const companyRecommendationPending = jobs.some((job) => job.jobType === "company_recommendations" && (job.status === "queued" || job.status === "running"));
  const companySourceSetupPending = jobs.some((job) => job.jobType === "company_source_setup" && (job.status === "queued" || job.status === "running"));
  ...
  return <div className="page-shell automation-page">
    ...
    {!advanced ? <>
        ...
        <div className="section-heading">
          ...
          <form action={runCompanyRecommendations}>
            <CompanyDiscoverySubmit
              ...
              running={companyRecommendationPending}
      ...
      <section className="standard-automation-panel discovery-setup-panel">
        <div className="standard-profile-summary"><span className="settings-icon"><Building2 size={18} /></span><div><p className="eyebrow">CONNECTABLE COMPANY SOURCES</p><h2>{pick(locale, "AI 精确连接公司招聘页", "AI-connectable company sources")}</h2><p>{pick(locale, "直接搜索并连接使用 Greenhouse、Lever 或 Ashby 的公开公司招聘页。普通官网推荐不会出现在这里。", "Search for and connect public company job boards hosted on Greenhouse, Lever, or Ashby. General careers-page recommendations stay in Standard mode.")}</p></div><form action={runCompanySourceSetup}><CompanyDiscoverySubmit disabled={!companyWebReady || !titles.length} running={companySourceSetupPending} label={pick(locale, "搜索并连接", "Search and connect")} pendingLabel={pick(locale, "正在搜索可连接来源…", "Searching connectable sources…")} /></form></div>
