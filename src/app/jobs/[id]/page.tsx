...
import { applicationEvents, applications, applicationStatuses, appSettings, jobs, jobMatches, materials } from "@/db/schema";
...
import { analyzeJobMatch } from "./ai-actions";
...
export default async function JobDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ coverLetter?: string; tailored?: string; outputLanguage?: string }> }) {
  ...
  const [rows, matches, statuses, aiSettingsRows, eventRows, materialRows] = batched;
  const row = rows[0];
  if (!row) notFound();
    ...
    <div className="page-shell">
      ...
      <header className="job-detail-header">
        ...
        <div className="header-actions">
          ...
          <form action={analyzeJobMatch}><input name="jobId" type="hidden" value={row.job.id} /><button className="button button-primary" disabled={!aiSettings?.aiEnabled || !keyConfigured} title={!aiSettings?.aiEnabled || !keyConfigured ? pick(locale, "先在设置中启用 AI 并配置当前提供商 Key", "Enable AI and configure the selected provider key first") : undefined}><Sparkles size={16} />{pick(locale, match?.modelName ? "重新分析" : "分析匹配度", match?.modelName ? "Analyze again" : "Analyze match")}</button></form>
        </div>
      </header>
