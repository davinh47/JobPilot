...
import { analyzeCandidateProfile, saveCandidateContext } from "@/lib/profile-analysis";
...
export async function analyzeProfileAction(formData: FormData) {
  ...
  if (!user) return;
  await saveCandidateContext(user.id, context.data);
  await analyzeCandidateProfile(user.id).catch(() => undefined);
  revalidatePath("/profile");
...
export async function analyzeProfileFromResumeAction() {
  ...
  try {
    await analyzeCandidateProfile(user.id);
  } catch {
    outcome = "failed";
  }
  revalidatePath("/profile");
  revalidatePath("/automation");
  redirect(`/profile?analysis=${outcome}`);
}
