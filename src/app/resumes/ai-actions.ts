...
import { queueResumeParse, queueResumeTranslation } from "@/lib/background-queue";
...
export async function restructureResume(input: { resumeId: string; locale: "zh" | "en"; sectionTemplate?: PlatformResume["sections"] }) {
  ...
  if (context.settings.workerEnabled === false) return { ok: false as const, error: checked.data.locale === "en" ? "Background jobs are disabled. Enable them in Sources & automation first." : "后台任务已关闭，请先在“岗位来源与自动化”中开启。" };
  ...
  const requestedTemplate = checked.data.sectionTemplate ?? (isPlatformResume(latest.structuredContentJson) ? normalizePlatformResume(latest.structuredContentJson).sections : undefined);
  const sectionTemplate = requestedTemplate?.map((section) => ({
    id: section.id,
    type: section.type,
    title: section.title,
    content: "",
  ...
  const queued = await queueResumeParse({ userId: context.resume.userId, resumeId: context.resume.id, sourceVersionId: latest.id, locale: checked.data.locale, sectionTemplate });
  revalidatePath(`/resumes/${context.resume.id}/edit`);
  revalidatePath(`/resumes/${context.resume.id}/preview`);
  revalidatePath("/resumes");
  return { ok: true as const, queued: true as const, backgroundJobId: queued.jobId };
}
