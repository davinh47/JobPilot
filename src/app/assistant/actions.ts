...
import { agentRuns, appSettings, experienceEvidence, notifications, resumes, resumeVersions } from "@/db/schema";
...
function isProjectRequest(message: string) {
  return /项目|project/i.test(message) && /添加|增加|补充|写|润色|修改|add|create|write|polish|edit/i.test(message);
}

function isProjectEditRequest(message: string) {
  return /修改|更新|改写|重写|润色|优化|edit|update|rewrite|revise|polish|improve/i.test(message);
}
...
export async function askJobPilotAssistant(input: { messages: AssistantChatMessage[]; pathname: string; locale: "zh" | "en" }) {
  ...
  const latestUserMessage = conversation[latestUserIndex]?.content ?? "";
  const activeIntent = resolveAssistantActiveIntent(conversation, latestUserIndex);
  const recentUserText = conversation
    .slice(Math.max(0, latestUserIndex - 5), latestUserIndex + 1)
    .filter((message) => message.role === "user")
    .map((message) => message.content)
    .join("\n");
  const projectRequest = isProjectRequest(latestUserMessage) || activeIntent === "resume_project";
  const resumeAdviceRequest = isResumeAdviceRequest(latestUserMessage) || activeIntent === "resume_advice";
  const resumeSyncRequest = isResumeSyncRequest(latestUserMessage) || activeIntent === "resume_sync";
...
- Return exactly one translation for every supplied unit and no others.`,
      ...
      await db.transaction(async (tx) => {
        ...
        if (settings.notificationsEnabled) {
          await tx.insert(notifications).values({
            ...
            titleZh: "双语简历同步草稿已完成",
            titleEn: "Bilingual resume sync draft ready",
            bodyZh: `已比较“${source.resume.title}”与“${target.resume.title}”，请在助手中检查并确认 ${drafts.length} 项修改。`,
            bodyEn: `“${source.resume.title}” and “${target.resume.title}” were compared. Review and confirm ${drafts.length} change${drafts.length === 1 ? "" : "s"} in the assistant.`,
            entityType: "resume",
            entityId: target.resume.id,
          }).run();
        }
  ...
  const run = await db.insert(agentRuns).values({
    ...
    modelProvider: settings.aiProvider,
    modelName: model,
    promptVersion: "jobpilot-assistant-v3",
    inputRefsJson: targetResume ? [{ type: "resume", id: targetResume.id }] : [],
    startedAt: new Date(),
  }).returning().get();

  const authoritativeUserText = conversation.filter((message) => message.role === "user").map((message) => message.content).join("\n");
...
- Continue the active JobPilot task instead of restarting or repeating a generic feature description. Switch tasks only when the user clearly introduces a different JobPilot workflow.
...
- Be concise and natural. Ask only for information that is genuinely missing, preferably one focused question at a time.
...
- If a target role or job description is needed for useful advice and none was provided, ask for it.
...
If facts are insufficient, return no drafts and ask short targeted questions. ${aiLanguageInstruction(checked.data.locale)}`,
    ...
    await db.transaction(async (tx) => {
      ...
      if ((projectRequest || resumeAdviceRequest) && settings.notificationsEnabled) {
        await tx.insert(notifications).values({
          userId: user.id,
          notificationType: "ai_task_complete",
          titleZh: response.projectDrafts.length || response.skillDrafts.length ? "助手修改草稿已完成" : "助手简历分析已完成",
          titleEn: response.projectDrafts.length || response.skillDrafts.length ? "Assistant edit draft ready" : "Assistant resume review ready",
          bodyZh: targetResume ? `“${targetResume.title}”的结果已保留在 JobPilot 助手中。` : "AI 结果已保留在 JobPilot 助手中。",
          bodyEn: targetResume ? `The result for “${targetResume.title}” is available in JobPilot Assistant.` : "The AI result is available in JobPilot Assistant.",
          entityType: targetResume ? "resume" : "agent_run",
          entityId: targetResume?.id ?? run.id,
        }).run();
      }
    });
