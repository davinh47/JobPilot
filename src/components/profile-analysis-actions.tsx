...
export function ProfileAnalysisActions({
  ...
  return <div className="profile-analysis-actions">
    ...
    <button
      ...
      disabled={!canAnalyze || pending || backgroundPending}
      name="profileAction"
      type="submit"
      value="analyze"
    >
      {isAnalyzing || backgroundPending ? <LoaderCircle className="spin" size={16} /> : <Sparkles size={16} />}
      {isAnalyzing || backgroundPending ? analyzePendingLabel : analyzeLabel}
    </button>
  </div>;
}
