...
export function ResumeEditor({ locale, resumeId, expectedVersionId, resumeLanguage = locale, resumeGroupId = "", title: initialTitle, content: initialContent, aiEnabled, jobs, initialOptimization = null, initialStructureQueued = false, savedChangeSummary, mode = "edit" }: { locale: Locale; resumeId?: string; expectedVersionId?: string; resumeLanguage?: "zh" | "en"; resumeGroupId?: string; title: string; content: PlatformResume; aiEnabled: boolean; jobs: JobOption[]; initialOptimization?: ResumeOptimizationResult | null; initialStructureQueued?: boolean; savedChangeSummary?: string | null; mode?: "create" | "edit" }) {
  ...
  const [structureQueued, setStructureQueued] = useState(initialStructureQueued);
  ...
  return <form action={action} className="resume-platform-editor">
    ...
    {aiEnabled && resumeId ? <><section className="resume-ai-workbench resume-structure-workbench"><div className="resume-ai-heading"><span><FileText size={18} /></span><div><p className="eyebrow">AI STRUCTURE</p><h2>{text("按原件重新整理结构", "Reorganize from source")}</h2><p>{text("重新读取上传原文，并按照你当前设置的模块和名称创建一个可检查的新版本。", "Re-read the uploaded source and create a reviewable version using your current sections and labels.")}</p></div></div><div className="resume-ai-controls"><button className="button button-secondary" disabled={structurePending || aiPending || structureQueued} onClick={requestRestructure} type="button"><WandSparkles size={16} />{structurePending ? text("正在提交…", "Queueing…") : structureQueued ? text("已在后台整理", "Running in background") : text("AI 重新整理结构", "AI reorganize structure")}</button></div></section><section className="resume-ai-workbench"><div className="resume-ai-heading"><span><Sparkles size={18} /></span><div><p className="eyebrow">JOB-SPECIFIC AI</p><h2>{text("按申请岗位优化", "Optimize for an application")}</h2><p>{text("任务会在后台运行，完成后通过通知返回审阅。", "Runs in the background and notifies you when the proposal is ready.")}</p></div></div>{jobs.length ? <div className="resume-ai-controls"><label>{text("目标岗位", "Target job")}<span className="select-with-icon"><BriefcaseBusiness size={16} /><select onChange={(event) => { setJobId(event.target.value); setOptimizationQueued(false); }} value={jobId}>{jobs.map((job) => <option key={job.id} value={job.id}>{job.label}</option>)}</select><ChevronDown size={15} /></span></label><button className="button button-primary" disabled={aiPending || structurePending || structureQueued || !jobId || optimizationQueued} onClick={requestOptimization} type="button"><Sparkles size={16} />{aiPending ? text("正在提交…", "Queueing…") : optimizationQueued ? text("已在后台分析", "Running in background") : text("生成优化建议", "Generate suggestions")}</button></div> : <p className="inline-empty"><BriefcaseBusiness size={17} />{text("将岗位加入申请进度后，即可按 JD 优化简历。", "Add a job to the pipeline to optimize against its description.")}</p>}</section></> : null}

    {aiError ? <p className="form-error" role="alert">{aiError}</p> : null}
    {structureQueued ? <p className="form-success" role="status">{text("结构整理任务已提交。你可以继续编辑或前往其他页面，完成后 JobPilot 会在通知中提醒你。", "The structure task is running in the background. You can keep editing or leave this page; JobPilot will notify you when it is ready.")}</p> : null}
