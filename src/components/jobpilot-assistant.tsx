...
export function JobPilotAssistant({ locale, initialOpen = false }: { locale: Locale; initialOpen?: boolean }) {
  ...
  const markAssistantRead = useCallback(() => {
    setHasUnread(false);
    void fetch("/api/assistant/status", { method: "POST" }).catch(() => undefined);
  }, []);
    ...
    void fetch("/api/assistant/status", { cache: "no-store" })
      ...
      .then((result) => {
        ...
        if (result) {
          ...
          setHasUnread(Boolean(result.assistantUnread) && !openRef.current);
  ...
  return (
    <>
      {!open ? <button aria-label={text("打开 JobPilot 助手", "Open JobPilot Assistant")} className="assistant-launcher" data-tour="assistant-launcher" onClick={openAssistant} title={text("JobPilot 助手", "JobPilot Assistant")} type="button"><MessageCircleMore size={22} />{hasUnread ? <span aria-label={text("有新的助手消息", "New assistant message")} className="assistant-unread-dot" /> : null}</button> : null}
      {open ? <aside aria-label={text("JobPilot 助手", "JobPilot Assistant")} className="assistant-drawer">
        <header className="assistant-header" data-tour="assistant-launcher">
          ...
          <span className="assistant-header-actions">
            ...
            <button aria-label={text("关闭助手", "Close assistant")} className="assistant-icon-button" onClick={() => { openRef.current = false; setOpen(false); }} title={text("关闭", "Close")} type="button"><X size={19} /></button>
          </span>
        </header>
        ...
        <div aria-live="polite" className="assistant-messages">
          ...
          {pending && !applyingId ? <div className="assistant-thinking"><span />{text("思考中…", "Thinking…")}</div> : null}
