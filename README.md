...
For a local workspace, run JobPilot on port `3000`. For the Vercel edition, use its HTTPS origin (the current public edition is `https://try-jobpilot.vercel.app`), then open **Settings → Chrome one-click saver**:

1. Download and unzip the packaged extension.
2. Open `chrome://extensions`.
3. Enable **Developer mode** and select **Load unpacked**.
4. The Vercel download is preconfigured with its deployment origin, so you only need to paste the pairing token shown by the same JobPilot account. Local or open-source builds show the **JobPilot URL** field; enter the origin without `/extension`, `/matches`, or an API path. Allow Chrome's host permission prompt for a hosted origin if it appears.
