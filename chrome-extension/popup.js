...
async function captureJobPage() {
  ...
  const description = first(["[data-automation='jobAdDetails']", ".jobs-description__content", ".show-more-less-html__markup", "[class*='job-description']", "main", "article"]);
  return {
    url: canonical,
    capturedHtml: document.documentElement.outerHTML.slice(0, 1500000),
    capturedText: (description || document.body.innerText).slice(0, 120000),
    hints: {
      title: first(["[data-automation='job-detail-title']", "h1", ".top-card-layout__title"]),
      companyName: first(["[data-automation='advertiser-name']", ".job-details-jobs-unified-top-card__company-name", ".topcard__org-name-link", "[class*='company-name']"]),
      location: first(["[data-automation='job-detail-location']", ".job-details-jobs-unified-top-card__bullet", ".topcard__flavor--bullet", "[class*='location']"]),
...
document.querySelector("#pair").addEventListener("click", async () => {
  ...
  if (!pairingToken || !baseUrl) return showStatus("Enter the local URL and pairing token.", true);
...
document.querySelector("#save").addEventListener("click", async (event) => {
  ...
  try {
    ...
    const response = await fetch(`${stored.baseUrl}/api/extension/jobs`, { method: "POST", headers: { Authorization: `Bearer ${stored.pairingToken}`, "Content-Type": "application/json" }, body: JSON.stringify(captured) });
